...
void md_parseToCorto(
    corto_object destination,
    const char *text)
{

    if (destination == root_o || destination == NULL) {
        corto_throw("must specify a destination different from root");
        return;
    }
    ...
    hoedown_document *parser = hoedown_document_new(renderer, HOEDOWN_EXT_BLOCK, 16);
...
int cortomain(int argc, char *argv[]) {
    CORTO_UNUSED(argc);
    CORTO_UNUSED(argv);
    corto_load_register("md", md_parseDoc, NULL);
    return 0;
}
